...
protocol VideoPlayerDelegate : AnyObject {
    ...
    func didFinishRecordVideo(url : URL?, error: Error?)
}
...
class VideoPlayerView : UIView {
    ...
    private func updateFrameVideoPlayer(){
        ...
        var frameOffset : CGRect {
            if UIScreen.main.bounds.width > UIScreen.main.bounds.height {
                let sizeOffset = CGSize(width: size.height * videoSize.width / max(videoSize.height, 1) , height: size.height)
                return .init(origin: .init(x: (size.width - sizeOffset.width) / 2 , y: 0), size: sizeOffset)
            }
            let sizeOffset = CGSize(width: size.width, height: size.width * videoSize.height / max(videoSize.width, 1))
            return .init(origin: .init(x: 0 , y: (size.height - sizeOffset.height) / 2), size: sizeOffset)
        }
        self.videoPlayerLayer.frame = frameOffset
        self.videoPlayerContainer.contentSize = self.videoPlayerContainer.frame.size
    }
